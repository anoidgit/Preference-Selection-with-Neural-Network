...
real dot_vec(real* v1, real* v2, int vsize){
    real rs = 0.0;
    int a = 0;
    for (a = 0; a < vsize; a++){
        rs += v1[a] * v2[a];
...
real valid(){
    ...
    real* unk_context = (real*)calloc((vector_size + 1), sizeof(real));
    int num_rare_words = vocab_size < 100 ? vocab_size : 100;
    real* unk_emb = (real*)calloc((vector_size), sizeof(real));
    long long a, b;
    for (a = vocab_size - num_rare_words; a < vocab_size; a++) {
        for (b = 0; b < (vector_size + 1); b++) {
            unk_vec[b] += W[a * (vector_size + 1) + b] / num_rare_words;
    ...
    if (model == 2) // Save "word + context word" vectors (without bias)
        for (b = 0; b < vector_size; b++) unk_emb[b] = unk_vec[b] + unk_context[b];
    else
        for (b = 0; b < vector_size; b++) unk_emb[b] = unk_vec[b];

    // valid
    unsigned int correct = 0, total = 0;
    FILE *fvalid;
    fvalid = fopen("cache/glove/valid.txt", "r");
    if (fvalid == NULL) {fprintf(stderr, "Unable to open file %s.\n", "cache/glove/valid.txt"); return 1.0;}
    real* v_emb = (real*)calloc((vector_size), sizeof(real));
    real* np_emb = (real*)calloc((vector_size), sizeof(real));
    real* nn_emb = (real*)calloc((vector_size), sizeof(real));
    while(!feof(fvalid)){
        long long vid, npid, nnid;
        if (fscanf(fvalid, "%lld,%lld,%lld", &vid, &npid, &nnid) == 0) return 1.0;
        if (model == 2){
            if (vid >= 0) for (b = 0; b < vector_size; b++) v_emb[b] = W[vid * (vector_size + 1) + b] + W[(vocab_size + vid) * (vector_size + 1) + b];
            else for (b = 0; b < vector_size; b++) v_emb[b] = unk_emb[b];
            if (npid >= 0) for (b = 0; b < vector_size; b++) np_emb[b] = W[npid * (vector_size + 1) + b] + W[(vocab_size + npid) * (vector_size + 1) + b];
            else for (b = 0; b < vector_size; b++) np_emb[b] = unk_emb[b];
            if (nnid >= 0) for (b = 0; b < vector_size; b++) nn_emb[b] = W[nnid * (vector_size + 1) + b] + W[(vocab_size + nnid) * (vector_size + 1) + b];
            else for (b = 0; b < vector_size; b++) nn_emb[b] = unk_emb[b];
        }
        else{
            if (vid >= 0) for (b = 0; b < vector_size; b++) v_emb[b] = W[vid * (vector_size + 1) + b];
            else for (b = 0; b < vector_size; b++) v_emb[b] = unk_emb[b];
            if (npid >= 0) for (b = 0; b < vector_size; b++) np_emb[b] = W[npid * (vector_size + 1) + b];
            else for (b = 0; b < vector_size; b++) np_emb[b] = unk_emb[b];
            if (nnid >= 0) for (b = 0; b < vector_size; b++) nn_emb[b] = W[nnid * (vector_size + 1) + b];
            else for (b = 0; b < vector_size; b++) nn_emb[b] = unk_emb[b];
        }
        real sp = cos_vec(v_emb, np_emb, vector_size);
        real np = cos_vec(v_emb, nn_emb, vector_size);
        if (sp >= np) correct ++;
        total ++;
    }
    free(unk_vec);
    ...
    free(nn_emb);
    fclose(fvalid);
    if (total > 0) return (real)correct/(real)total;
    return 1.0;
}
...
int train_glove() {
    ...
    for (b = 0; b < num_iter; b++) {
        ...
        fprintf(stderr, "%s, iter: %03d, cost: %lf, valid acc: %lf\n", time_buffer,  b+1, total_cost/num_lines, cvacc);
        ...
        if (cvacc >= bvacc){
            ...
            bvacc = cvacc;
            ab_acc = 0;
            fprintf(stderr, "New best dev model saved with %lf.\n", cvacc);
        }
        else{
            ab_acc ++;
            if (ab_acc >= early_stop){
                fprintf(stderr, "Early stop with acc: %lf.\n", bvacc);
                break;
            }
        }
